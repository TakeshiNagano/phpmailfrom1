...
define('SMTP', envBool('CONTACT_SMTP_ENABLED', true)); //smtpを使って送信の場合true
define('MAILHOST', envString('CONTACT_SMTP_HOST', 'ham1001.secure.ne.jp')); //cpi smtp
define('SMTPAUTH', envBool('CONTACT_SMTP_AUTH', true));
define('SMTPUSER', envString('CONTACT_SMTP_USER', 't-murakami@shinkibizpro.co.jp'));
define('SMTPPASW', envString('CONTACT_SMTP_PASS', ''));
$smtpSecure = strtolower(envString('CONTACT_SMTP_SECURE', 'ssl'));
if ($smtpSecure === 'false' || $smtpSecure === 'none' || $smtpSecure === '0') {
    $smtpSecure = '';
}
define('SMTPSEC', $smtpSecure); //'tls','ssl',false
define('SMTPPORT', (int) envString('CONTACT_SMTP_PORT', '465')); //cpi 465

define('RECAPTCHA_SITE_KEY', envString('CONTACT_RECAPTCHA_SITE_KEY', ''));
define('RECAPTCHA_SECRET_KEY', envString('CONTACT_RECAPTCHA_SECRET_KEY', ''));

// キーが設定されていない場合は、環境変数等で有効になっていても強制的に無効化する
$recaptchaEnabled = envBool('CONTACT_RECAPTCHA_ENABLED', (RECAPTCHA_SITE_KEY !== '' && RECAPTCHA_SECRET_KEY !== ''));
...
define('RECAPTCHA_ACTION', envString('CONTACT_RECAPTCHA_ACTION', 'contact_submit'));
$recaptchaMinScore = (float) envString('CONTACT_RECAPTCHA_MIN_SCORE', '0.5');
